...
mkdir -p build
{
    ...
    if "$CXX" "${FLAGS[@]}" tests/tests.cpp tests/matcher_tests.cpp tests/rb_tree_tests.cpp tests/journaler_tests.cpp tests/sequencer_tests.cpp -o build/titan_tests; then
        echo "COMPILE_OK"
        echo "== running tests =="
        ./build/titan_tests
        echo "TEST_EXIT=$?"
