...
| **Matching Engine (using PIN)** | ✅ | v1.1.2–v1.1.4 (+hardening v1.3.3, +cache-locality v1.4.10) | 2026-07-14 | PIN book, dense-slab O(1) id index, intrusive RB-tree price index, LIMIT/MARKET/IOC + partial fills. **Zero-crash under pool/arena exhaustion** (REJECTED event). **v1.4.10 cache-locality**: `SlabEntry` widened to 32 B `alignas(32)` caching price/qty/side → cancel satisfied from one slab line (no `at()` chase into the cold PIN slot payload; profile said that was ~27%). +25% throughput on the cancel-heavy bench |
...
| Publish Data / Trade Reporter | ✅ | v1.4.4 (+snapshot v1.4.7) | 2026-07-14 | **UDP multicast** (`net/udp_publisher.hpp`): non-blocking, `IP_MULTICAST_IF/TTL/LOOP`, raw-binary (zero JSON), MTU-safe chunking, drop-on-full. **Two channels**: incremental TradeEvents (T4, :30001, each tagged `feed_seq`) + L2 snapshots (T5, `send_raw`, :30002). Verified: 122,223 trades + 19 L2 snapshots, zero loss |
...
| **Snapshot & Gap-Fill** | 🟡 | v1.4.6 (wired v1.4.7) | 2026-07-14 | L2 snapshot primitive (`book/snapshot.hpp`): triple-buffer pool (seq_cst `in_use` + `published`), TSan-proven. **WIRED**: `feed_seq` on every TradeEvent (gap detect); `OrderBook::serialize_l2` (RB-index walk → L2 aggregates, depth-limited to 1 MTU); Matcher emits every K=10k orders (modulo, no timer); **T5 snapshot thread** multicasts on a **separate channel** (:30002). Dual-feed verified: 19 valid L2 snapshots, monotonic feed_seq. Gap-fill = feed emitted; **client-side resync loop not built** |
...
| v1.4.9 | 2026-07-14 | `profile.sh` (perf preferred; gprof fallback — perf absent on this WSL2 kernel, no linux-tools/sudo). Finding: engine is **memory/pointer-chasing bound, not math bound** — fill arithmetic <3%, RB-tree traversal ~10%, PIN-node slot access + cancel-path indirection ~60% |
| v1.4.10 | 2026-07-14 | Cache-locality: cancel/lookup satisfied from a 32-B `alignas(32)` `SlabEntry` (cached price/qty/side; `remaining` synced on partial fill via `note_partial_fill`) — no chase into the cold PIN slot payload. matcher_bench (55% cancels): **203.9 → 162.6 ns/msg (-20%), 4.90 → 6.15 M/s (+25%)**. Wire-to-wire (cross-only blaster, no cancels): unchanged. ASan 37/37 green |
| v1.4.11 | 2026-07-14 | PIN_Node metadata-first layout inversion: control block (`occupancy_mask`+head/tail+FIFO links) `alignas(64)` at offset 0, `slots` payload after — so `occupancy_mask`+`head_slot` share the front cache line (was ~2 lines apart). Correct, ASan 37/37 green, zero memory/mechanics change. **Throughput delta below the WSL2 noise floor** (ingest 8.9–20.4 M/s across identical configs); benefit is cold-node deep-book crossing, which neither bench (hot single-node blaster / cancel-heavy matcher_bench) + WSL variance can measure |
| *(uncommitted)* docs | 2026-07-14 | `ARCHITECTURE.md` — flagship design doc (firm-style: invariants, topology, memory-ordering model, perf w/ methodology, verification gates, non-goals). Metrics grounded in-repo; datagram count corrected 45→36 (40 B TradeEvent) |
