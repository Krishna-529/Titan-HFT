...
// -----------------------------------------------------------------------------
// PIN_Node_HC: experimental HOT/COLD layout variant. Identical interface and
// method bodies to PIN_Node; the ONLY difference is member order -- the hot
// metadata (occupancy mask, FIFO links, head/tail, level-chain pointers) is
// placed FIRST so that touching nodes_[idx] loads the traversal/occupancy fields
// in the node's leading cache line, and the cold 2.5 KB slots[] payload sits at
// the end. Benchmarked against PIN_Node via the A/B harness (bench/ab_bench.cpp).
// -----------------------------------------------------------------------------
struct alignas(64) PIN_Node_HC {
    static constexpr std::uint32_t CAPACITY = 64;
    static constexpr std::uint8_t  NIL_SLOT = 0xFF;

    // --- HOT: metadata first (leading cache lines) ---
    std::uint64_t occupancy_mask;
    std::array<std::uint8_t, CAPACITY> next_in_time;
    std::array<std::uint8_t, CAPACITY> prev_in_time;
    std::uint8_t  head_slot;
    std::uint8_t  tail_slot;
    std::uint32_t next;
    std::uint32_t prev;

    // --- COLD: order payloads last ---
    std::array<Order, CAPACITY> slots;
    ...
    std::uint32_t insert(const Order& o) noexcept {
        const std::uint64_t free = ~occupancy_mask;
        if (free == 0) return INVALID_INDEX;
        const std::uint32_t slot = static_cast<std::uint32_t>(__builtin_ctzll(free));
        slots[slot] = o;
        occupancy_mask |= (std::uint64_t(1) << slot);

        const std::uint8_t s = static_cast<std::uint8_t>(slot);
        next_in_time[s] = NIL_SLOT;
        prev_in_time[s] = tail_slot;
        if (tail_slot == NIL_SLOT) head_slot = s;
        else                       next_in_time[tail_slot] = s;
        tail_slot = s;
        return slot;
    }
    ...
    bool remove(std::uint32_t slot) noexcept {
        ...
        const std::uint64_t bit = std::uint64_t(1) << slot;
        if ((occupancy_mask & bit) == 0) return false;
        occupancy_mask &= ~bit;
...
static_assert(alignof(PIN_Node_HC) == 64,                "PIN_Node_HC must be cache-line aligned");
static_assert(std::is_trivially_copyable_v<PIN_Node_HC>, "PIN_Node_HC must be trivially copyable");
static_assert(std::is_standard_layout_v<PIN_Node_HC>,    "PIN_Node_HC must be standard layout");
